...
class BackupExportView(context: Context, attrs: AttributeSet, style: Int) extends LinearLayout(context, attrs, style) with ViewHelper {
  ...
  private def backupData: Future[Unit] = {
    ...
    (for {
      ...
      ShareCompat.IntentBuilder.from(context.asInstanceOf[Activity])
        .setType("application/zip")
        .setStream(AndroidURIUtil.fromFile(file).uri)
        .setChooserTitle(R.string.pref_account_backup_title)
        .startChooser()
